...
const ASSET_REPORT_PIPELINE = [
  ...
  ...lookupStages(),
  {
    $addFields: {
      warrantyDaysRemaining: {
        $cond: [
          { $ifNull: ["$warrantyEnd", false] },
          { $divide: [{ $subtract: ["$warrantyEnd", "$$NOW"] }, 1000 * 60 * 60 * 24] },
          null,
        ],
      },
    },
  },
  {
    $addFields: {
      ...
      isAdMember: TRUTHY_EXPR("$adMember"),
      hasAntivirus: {
        $ne: [{ $trim: { input: { $ifNull: ["$antivirusInstalled", ""] } } }, ""],
      },
        ...
        $round: [
          {
            $multiply: [
              {
                $divide: [
                  { $sum: [{ $toInt: "$isAdMember" }, { $toInt: "$hasAntivirus" }, { $toInt: "$isOsLicensed" }] },
                  3,
                ],
              },
              100,
  ...
  },
  {
    $project: {
      ...
      laptopGeneration: str("$laptopGeneration"),
      graphicsCard: str("$graphicsCard"),
      ram: str("$ram"),
      storage: str("$storage"),
      ...
      miscAccessories: str("$miscAccessories"),
      operatingSystem: str("$operatingSystem"),
      operatingSystemLicense: str("$operatingSystemLicense"),
      isOsLicensed: 1,
      adMember: str("$adMember"),
      isAdMember: 1,
      antivirusInstalled: str("$antivirusInstalled"),
      hasAntivirus: 1,
      remoteSoftware: str("$remoteSoftware"),
      ...
      previousOwner: str("$previousOwner"),
      notes: str("$notes"),
      complianceScore: 1,
      createdDate: 1,
    },
