...
import { LicenseCategory, type ILicenseCategory } from "../../models/LicenseCategory";
...
export const LICENSE_CATEGORY_IMPORT_TEMPLATE_COLUMNS = ["Name", "Description"];
...
export type MappedLicenseCategoryRow = {
  rowIndex: number;
  mapped: MappedFields;
...
function diffAgainstExisting(mapped: MappedFields, existing: ILicenseCategory): string[] {
...
async function classifyRows(rawRows: Record<string, string>[], organizationId: string): Promise<MappedLicenseCategoryRow[]> {
  // Hard-delete module - no isDeleted filter, every remaining doc is "current".
  const existingCategories = await LicenseCategory.find({ organization: organizationId });
  ...
  return rawRows.map((row, rowIndex) => {
    ...
    let classification: MappedLicenseCategoryRow["classification"];
...
export const previewLicenseCategoryImport = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!hasAnyRecognizableColumn(rawRows)) {
    throw new ApiError(400, "This file doesn't look like a license category CSV - no Name column was found. Try the template.");
  }
...
export const confirmLicenseCategoryImport = asyncHandler(async (req: Request, res: Response) => {
  const organizationId = req.organization!._id;
  const rows = (req.body.rows as MappedLicenseCategoryRow[]) ?? [];
  ...
  for (const row of newRows) {
    try {
      await licenseCategoriesService.createLicenseCategory(
        { name: row.mapped.name, ...buildPayload(row.mapped, true) },
        organizationId
      );
      added += 1;
    } catch (err) {
  ...
  for (const row of updatedRows) {
    try {
      ...
      await licenseCategoriesService.updateLicenseCategory(row.existingId, buildPayload(row.mapped, false), organizationId);
      updated += 1;
    } catch (err) {
      errors.push(`Row ${row.rowIndex + 1}: ${err instanceof Error ? err.message : "unknown error"}`);
    }
  ...
  await recordImportBatch({ organizationId, module: "LicenseCategory", userId: req.user?.id, fileName, counts, errors });

  ok(res, { ...counts, errors }, "Import complete");
});

export const downloadLicenseCategoryTemplate = asyncHandler(async (_req: Request, res: Response) => {
  res.setHeader("Content-Type", "text/csv");
  res.setHeader("Content-Disposition", 'attachment; filename="license-category-import-template.csv"');
  res.send(LICENSE_CATEGORY_IMPORT_TEMPLATE_COLUMNS.join(",") + "\n");
});

export const getLicenseCategoryImportHistory = asyncHandler(async (req: Request, res: Response) => {
