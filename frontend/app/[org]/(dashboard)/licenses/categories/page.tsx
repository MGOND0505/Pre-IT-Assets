      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            {canWrite && (
              <DropdownMenuItem render={<Link href={toOrgHref(`/licenses/categories/${row.original._id}/edit`)} />}>
                Edit
              </DropdownMenuItem>
            )}
            {canDelete && (
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {canCreate && (
          <MagneticButton>
            <Button render={<Link href={toOrgHref("/licenses/categories/add")} />}>Add category</Button>
          </MagneticButton>
        )}
      </div>
